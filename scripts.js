$.when($.ready).then(function () {
    ...
    function getCourses(q = "", topic = "", sort = "") {
        ...
        $.ajax({
            url: `https://smileschool-api.hbtn.info/courses?&q=${q.toLowerCase()}&topic=${topic}&sort=${sort}`,
            type: "GET",
            contentType: 'application/json',
            success: function (response) {
                let i = 1;
                const t_courses = Object.keys(response.courses).length
    ...
    $.ajax({
        ...
        contentType: 'application/json',
        success: function (response) {
            const quote1 = response[0];
            const quote2 = response[1];
    ...
    $.ajax({
        ...
        contentType: 'application/json',
        success: function (response) {
            const t_card1 = response[0];
            const t_card2 = response[1];
            const t_card3 = response[2];
            const t_card4 = response[3];
            const t_card5 = response[4];
            const t_card6 = response[5];
            const t_card7 = response[6];
    ...
    $.ajax({
        ...
        contentType: 'application/json',
        success: function (response) {
            const l_card1 = response[0];
            const l_card2 = response[1];
            const l_card3 = response[2];
            const l_card4 = response[3];
